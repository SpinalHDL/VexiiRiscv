    ...
    val peripheral = new ClockingArea(litexCd) {
      ...
      val aplic = withAPlic generate new Area {
        ...
        val s = withSupervisor generate new Area {
          ...
          val childSourceIds = 1 to 31
          val childInfos = APlicChildInfo(0, childSourceIds)
          val childSources = m.intc.createInterruptDelegation(childInfos)
          val childBundles = childSourceIds.zip(childSources.flags).map {
            case (id, childSource) => intc.mapUpInterrupt(id, childSource)
          }
