...
case class LsuL1Bus(p : LsuL1BusParameter) extends Bundle with IMasterSlave {
  ...
  /**
   * In short, when coherency is disabled :
   * tl.A << read.cmd + write.cmd
   * tl.D >> read.rsp + write.rsp
   *
   * when coherency is enabled :
   * tl.A << read.cmd
   * tl.B + failed probe.rsp >> probe.cmd
   * tl.C << probe.rsp that changed nothing + write.cmd
   * tl.D >> read.rsp + write.rsp
   * tl.E << read.ack
   */
  def toTilelink(probeInflightMax : Int = 2): tilelink.Bus = new Composite(this, "toTilelink"){
    ...
    val coherent = p.withCoherency generate new Area{
      ...
      //onC is tricky, as we need to retry failed probes
      val onC = new Area{
        ...
        // Feed the failed probe.rsp back to the probe.cmd
        when(rspFlow.valid && rspFlow.redo) {
          ...
          bus.b.ready := False //bus.b has low priority and is blocked.
        }

        // We have two tilelink.C transaction sources : probes responses which produced no changes, and writebacks from the cache
        // So we need to arbitrate between them.
