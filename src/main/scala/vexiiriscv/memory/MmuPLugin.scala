...
//            requireMmuLockup clearWhen (!status.mprv || priv.logic.csrs(0).m.status.mpp === 3)
//          } else {
...
//          PAGE_FAULT    := lineException || lineAllowUser && isSupervisor && !status.sum || !lineAllowUser && isUser
...
//
//        def cmd = setup.accessBus.cmd
//        val rspUnbuffered = setup.accessBus.rsp
...
//        cmd.unique          := False
//
//        setup.accessBus.translated.physical := address
//        setup.accessBus.translated.abord    := False
//
//        setup.accessBus.cancels := 0
//
