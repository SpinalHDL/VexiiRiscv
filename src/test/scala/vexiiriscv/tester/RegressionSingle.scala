...
/**
 * Specifies which set of tests should be generated by a RegressionSingle, as well as which traces should be generated by the testbenches
 */
...
object RegressionSingle extends App{
  def test(name : String, dutGen : => TestBenchDut, dutArgs : Seq[String], config : RegressionSingleConfig): Unit = {
    def gen = dutGen.setDefinitionName(s"VexiiRiscv_$name")
    if(config.withSim) {
  ...
  def test(ps : ParamSimple, dutArgs : Seq[String], config : RegressionSingleConfig): Unit = {
    if(config.cpuCount > 1 && ps.lsuL1Enable) ps.lsuL1Coherency = true
    test(s"${ps.hashCode()}c${config.cpuCount}", TestBench.makeDut(ps, config.cpuCount), dutArgs, config)
  }
  ...
  def test(args : Seq[String]): Unit = {
    ...
    assert(new scopt.OptionParser[Unit]("VexiiRiscv") {
      help("help").text("prints this usage text")
      opt[Int]("cpu-count") validate { v => if(v >= 1) success else failure("must be at least 1") } action { (v, c) => config.cpuCount = v }
